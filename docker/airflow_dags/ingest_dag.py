...
#          Run crypto.py → get live prices from CoinGecko

# Step 2 — produce_to_kafka why is it here - Airflow handles the scheduled batch ingestion part.
#          Run producer.py → send prices to Kafka topic

# Step 3 — log_pipeline_status why is it here - Together they form your complete pipeline.
#          Write a record to pipeline_logs table in PostgreSQL
#          → "pipeline ran at 10:00, 5 rows processed, success"

# Step 4 — alert_on_failure  why is it here - Alerts you if anything breaks in the pipeline.
#          Run crypto.py → get live prices from CoinGecko
#          Run producer.py → send prices to Kafka topic
#          Run consumer.py → read prices from Kafka topic
#          Run log_pipeline_status → write a record to pipeline_logs
#          → "pipeline ran at 10:00, 5 rows processed, success"
